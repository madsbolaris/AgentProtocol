...
DOTNET_DEST="$PROJECT_ROOT/dotnet/samples/agents/FunctionToolsAgent"

mkdir -p "$DOTNET_DEST"

# Copy the files
cp "$DOTNET_SRC/Program.cs" "$DOTNET_DEST/Program.cs"
cp "$DOTNET_SRC/Agent_Step03_UsingFunctionTools.csproj" "$DOTNET_DEST/FunctionToolsAgent.csproj"
...
cat > "$DOTNET_DEST/FunctionToolsAgent.csproj" << 'EOF'
...
PYTHON_DEST="$PROJECT_ROOT/python/samples/agents/function_tools_agent"

mkdir -p "$PYTHON_DEST"

# Create modified Python sample
cat > "$PYTHON_DEST/function_tools_agent.py" << 'EOF'
# Copyright (c) Microsoft. All rights reserved.

"""
Function Tools Agent Example
...
async def main() -> None:
    print("=== Microsoft Foundry - Function Tools Agent ===\n")
...
# Or from project root
python python/samples/agents/function_tools_agent/function_tools_agent.py
```
...
TS_DEST="$PROJECT_ROOT/typescript/samples/agents/function-tools-agent"

mkdir -p "$TS_DEST"

# Create TypeScript example (protocol-level since no SDK exists yet)
cat > "$TS_DEST/function-tools-agent.ts" << 'EOF'
/**
 * Function Tools Agent Example (TypeScript)
 *
...
{
  "name": "function-tools-agent-typescript",
  "version": "1.0.0",
  "description": "Function Tools Agent example showing Agent Protocol message structures",
  "main": "function-tools-agent.ts",
  "scripts": {
    "start": "ts-node function-tools-agent.ts",
    "build": "tsc function-tools-agent.ts"
  },
...
# Function Tools Agent (TypeScript - Protocol Example)
...
npx ts-node function-tools-agent.ts
```
...
echo -e "    python function_tools_agent.py"
echo -e ""
