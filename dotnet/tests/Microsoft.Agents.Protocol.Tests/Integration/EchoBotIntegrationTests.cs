...
///
///     # Then run tests
///     dotnet test --filter "FullyQualifiedName~EchoBotIntegrationTests"
/// </summary>
public class EchoBotIntegrationTests
{
    private readonly ITestOutputHelper _output;
    private static readonly Dictionary<string, string> EchoBotServers = new()
    {
    ...
    public EchoBotIntegrationTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    {
        var repoRoot = GetRepoRoot();
        var goldenPath = Path.Combine(repoRoot, "test-data", "results", "echobot", "json", $"{testName}-result.json");
    ...
    public static IEnumerable<object[]> GetTestData()
    {
        ...
        foreach (var language in EchoBotServers.Keys)
        {
    ...
    public async Task TestEchoBotAgainstGoldenFiles(string language, string testName, string inputFile)
    {
        /*
        ...
        _output.WriteLine($"   Server: {EchoBotServers[language]}");
        _output.WriteLine($"   Input: {Path.GetFileName(inputFile)}");
        _output.WriteLine($"{'='*70}");

        // Check server is running
        var baseUrl = EchoBotServers[language];
        ...
        catch (Exception e)
        {
            throw new SkipException(
                ...
                $"Start it with: ./scripts/start-all-echo-bots.sh");
        }
        ...
        catch (Exception e)
        {
            Assert.Fail(
                ...
                $"Make sure the echo bot is running: ./scripts/start-all-echo-bots.sh");
            return;
        }
    ...
    public async Task TestInputFieldNotInResponses()
    {
        ...
        foreach (var (lang, url) in EchoBotServers)
        {
