...
def verify_directory(shared_dir: Path, temp_dir: Path, pattern: str) -> Tuple[int, int, int]:
    ...
    Args:
        shared_dir: Directory with shared results (e.g., echobot/xml/)
        temp_dir: Directory with new results to compare (e.g., echobot/dotnet-temp/xml/)
    ...
    if args.generate_dotnet:
        ...
        try:
            ...
            result = subprocess.run(
                ["dotnet", "test", "--filter", "EchoBotIntegration"],
                cwd=dotnet_test_dir,
                capture_output=True,
                text=True
            )
