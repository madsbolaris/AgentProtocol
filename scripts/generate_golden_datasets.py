...
4. Optionally generates LLM recordings in test-data/llm-recordings/{sample}/
...
Usage:
    ...
    # Generate with LLM recording capture
    python scripts/generate_golden_datasets.py --record-llm

    # Specify custom inputs directory
    python scripts/generate_golden_datasets.py --inputs test-data/input/
...
class GoldenDatasetGenerator:
    ...
    def _find_dotnet_bot_directory(self, sample_name: str) -> Optional[Path]:
        ...
        Args:
            sample_name: Sample name (e.g., "echom365", "basic-m365")
    ...
    def _start_dotnet_bot(self, sample_name: str, port: int) -> Optional[subprocess.Popen]:
        ...
        Args:
            sample_name: Sample name (e.g., "echom365")
        ...
        try:
            ...
            # Enable LLM recording if requested
            if self.record_llm:
                env["RECORD_LLM"] = "true"
    ...
    def _extract_sample_name(self, bot_key: str) -> str:
        ...
        Returns:
            Sample name (e.g., "echom365", "basic-m365")
        """
        ...
        # Default to echom365 for base language keys
    ...
    def _group_bots_by_sample(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        Returns:
            ...
            Example: {"echom365": [{lang: "dotnet", config: {...}}, ...]}
        """
        samples = {}
    ...
    def generate_for_sample(
        ...
        if self.record_llm and sample_recordings_dir.exists():
    ...
    def generate_all(self, sample_filter: Optional[str] = None) -> None:
        """
        ...
        print("   SAMPLE_NAME=echom365 pytest python/microsoft-agents-xml/tests/")
...
Examples:
  ...
  python scripts/generate_golden_datasets.py --inputs test-data/input --results test-data/results
    ...
    parser.add_argument(
        "--record-llm",
        action="store_true",
        help="Record LLM interactions to test-data/llm-recordings/ (for samples that use LLM)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    # Generate
    generator.generate_all(sample_filter=args.sample)
