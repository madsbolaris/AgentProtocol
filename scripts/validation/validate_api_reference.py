...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Validate generated API reference documentation quality',
        epilog="""
Examples:
  # Validate default API reference directory
  python3 scripts/validation/validate_api_reference.py

  # Validate custom directory
  python3 scripts/validation/validate_api_reference.py --api-ref docs/api

  # Generate API reference first if missing:
  python3 scripts/codegen/generate_api_reference.py

Checks performed:
  - TypeSpec metadata pollution in docs
  - Missing manual content
  - Empty or placeholder descriptions
  - Broken documentation structure
  - Incomplete lists and examples
        """,
    ...
    parser.add_argument(
        ...
        help='Path to API reference directory (default: api-reference)'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    api_ref_dir = project_root / args.api_ref

    if not api_ref_dir.exists():
        print(f"❌ API reference directory not found: {api_ref_dir}")
        print()
        print("Expected location:")
        print(f"  {api_ref_dir.absolute()}")
        print()
        print("To generate API reference documentation:")
        print("  python3 scripts/codegen/generate_api_reference.py")
        print()
        print("Or skip this validation if you don't need API docs.")
        return 1

    # Check if directory is empty
    md_files = list(api_ref_dir.glob("**/*.md"))
    if not md_files:
        print(f"⚠️  API reference directory is empty: {api_ref_dir}")
        print()
        print("To generate API reference documentation:")
        print("  python3 scripts/codegen/generate_api_reference.py")
        return 1

    print(f"📁 Validating: {api_ref_dir}")
