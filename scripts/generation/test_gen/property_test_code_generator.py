...
def _generate_model_property_tests_python(model: Model, test_generator: PropertyTestGenerator) -> List[str]:
    ...
    for prop in testable_props:
        ...
        lines.append(f"    assert content.{to_snake_case(prop.name)} is not None")

        # Type-specific assertions
        if prop.type in ["int32", "integer", "int64"]:
            lines.append(f"    assert content.{to_snake_case(prop.name)} == int(test_value)")
        elif prop.type in ["float32", "float64", "float"]:
            lines.append(f"    assert abs(content.{to_snake_case(prop.name)} - float(test_value)) < 0.01")
        elif prop.type == "boolean":
            lines.append(f"    assert content.{to_snake_case(prop.name)} == (test_value.lower() == 'true')")
        else:
            lines.append(f"    assert content.{to_snake_case(prop.name)} == test_value")
