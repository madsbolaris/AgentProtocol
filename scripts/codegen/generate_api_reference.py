...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate API reference documentation from TypeSpec',
        epilog="""
Examples:
  # Generate API reference (with prompt)
  python3 scripts/codegen/generate_api_reference.py

  # Force generation without prompt
  python3 scripts/codegen/generate_api_reference.py --force

  # See what would be generated
  python3 scripts/codegen/generate_api_reference.py --dry-run

  # Custom TypeSpec directory
  python3 scripts/codegen/generate_api_reference.py --typespec path/to/typespec

This generates markdown documentation for:
  - All API endpoints (from routes.tsp)
  - All models (from messages.tsp, execution.tsp, etc.)
  - All enums

Output location: .generated/api-reference/
        """,
    ...
    parser.add_argument(
        ...
        help='Output directory (default: .generated/api-reference)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    parser.add_argument(
        '--force', '-f',
        action='store_true',
        help='Generate without confirmation prompt'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    typespec_dir = project_root / args.typespec
    output_dir = project_root / args.output

    # Validate TypeSpec directory exists
    if not typespec_dir.exists():
        print(f"❌ TypeSpec directory not found: {typespec_dir}")
        print()
        print("Expected location:")
        print(f"  {typespec_dir.absolute()}")
        print()
        print("Make sure you're running from project root.")
        return 1

    # Check if required TypeSpec files exist
    required_files = ['routes.tsp', 'messages.tsp']
    missing_files = [f for f in required_files if not (typespec_dir / f).exists()]
    if missing_files:
        print(f"❌ Required TypeSpec files not found:")
        for f in missing_files:
            print(f"  - {typespec_dir / f}")
        return 1

    # Show what will be done
    print("=" * 70)
    print("📚 API Reference Generator")
    print("=" * 70)
    print()
    print(f"TypeSpec directory: {typespec_dir}")
    print(f"Output directory:   {output_dir}")
    print()

    # Prompt for confirmation if not --force and not --dry-run
    if not args.force and not args.dry_run:
        if output_dir.exists():
            file_count = len(list(output_dir.glob("**/*.md")))
            if file_count > 0:
                print(f"⚠️  Output directory exists with {file_count} files")
                print()

        response = input("Generate API reference documentation? [y/N] ")
    ...
    # Parse TypeSpec
    try:
        type_parser = TypeSpecParser(str(typespec_dir))
        type_parser.parse_all()
    ...
    if args.dry_run:
        ...
        print()
        print(f"Endpoints: {len(type_parser.endpoints)}")
        print(f"Models:    {len(type_parser.models)}")
        print(f"Enums:     {len(type_parser.enums)}")
        print()
        print("Sample files that would be created:")
        for endpoint in list(type_parser.endpoints)[:5]:
            filename = f"{endpoint.method.lower()}-{endpoint.path.replace('/', '-').replace('{', '').replace('}', '')}.md"
            print(f"  - endpoints/{filename}")
        if len(type_parser.endpoints) > 5:
            print(f"  ... and {len(type_parser.endpoints) - 5} more endpoint files")
        print()
        print(f"Run without --dry-run to generate files in: {output_dir}")
        return 0

    # Generate markdown
    try:
        generator = MarkdownGenerator(type_parser, str(output_dir))
    ...
    print("  2. Validate: python3 scripts/validation/validate_api_reference.py")
    print("  3. Copy to docs/: cp -r .generated/api-reference/* docs/api-reference/")
    print()
