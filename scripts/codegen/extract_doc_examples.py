...
class CodeExtractor:
    ...
    def extract_python_examples(self) -> Dict[str, dict]:
        ...
        test_dirs = [
            self.repo_root / "python" / "microsoft-agents-protocol-xml" / "tests",
    ...
    def _extract_from_python_file(self, file_path: Path) -> Dict[str, dict]:
        ...
        for decorator_match in re.finditer(decorator_pattern, content):
            ...
            # Find code snippet marked with doc-example-start/end (only first one)
            snippet_pattern = r'#\s*doc-example-start(?::\s*(\w+))?\s*\n(.*?)\n\s*#\s*doc-example-end(?::\s*\1)?'
            snippet = re.search(snippet_pattern, content[func_start:], re.DOTALL)
    ...
    def _extract_from_csharp_file(self, file_path: Path) -> Dict[str, dict]:
        ...
        # Updated pattern to handle both sync and async methods
        method_pattern = r'public\s+(?:async\s+)?\w+(?:<[^>]+>)?\s+(\w+)\s*\('
        ...
        for attr_match in re.finditer(attr_start_pattern, content, re.DOTALL):
            ...
            # Find code snippet marked with doc-example-start/end (only first one)
            snippet_pattern = r'//\s*doc-example-start(?::\s*(\w+))?\s*\n(.*?)\n\s*//\s*doc-example-end(?::\s*\1)?'
            snippet = re.search(snippet_pattern, content[method_start:], re.DOTALL)
    ...
    def extract_typescript_examples(self) -> Dict[str, dict]:
        ...
        test_dirs = [
            self.repo_root / "typescript" / "packages" / "microsoft-agents-protocol-xml" / "tests",
            self.repo_root / "typescript" / "packages" / "agents-protocol-client" / "tests",
        ]
    ...
    def _extract_from_typescript_file(self, file_path: Path) -> Dict[str, dict]:
        ...
        for decorator_match in re.finditer(decorator_pattern, content):
            ...
            snippet_pattern = r'//\s*doc-example-start(?::\s*(\w+))?\s*\n(.*?)\n\s*//\s*doc-example-end'
            snippet = re.search(snippet_pattern, search_content, re.DOTALL)
...
Examples:
  ...
  python scripts/extract-doc-examples.py --verbose
    ...
    # Dry-run mode: show what would be saved without writing files
    if args.dry_run:
        print("\n" + "=" * 60)
        ...
        for example in examples.values():
            ...
            test_id = example["testId"]
            section = example["section"]
            extension = extractor._get_extension(language)
            filename = f"{test_id}_{section}.{extension}"
        ...
        print(f"\nWould generate {total_files + 1} files:")

        for language, filenames in sorted(files_by_language.items()):
            print(f"\n  {language}/ ({len(filenames)} files)")
