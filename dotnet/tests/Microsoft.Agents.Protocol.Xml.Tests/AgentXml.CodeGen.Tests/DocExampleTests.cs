...
<chat role=""user"" messageId=""msg-001"">
  <text>Hello, agent!</text>
            ...
            XunitAssert.Equal(ChatRole.User, message.Role);
        }
        ...
        public void TestContentValidation()
        {
            ...
            XunitAssert.NotNull(content.Text); // Text content must have text
            XunitAssert.True(content.Text.Length > 0); // Text cannot be empty
            XunitAssert.Equal("text", content.Kind); // Kind must match content type
            ...
            XunitAssert.True(result.validated);
        }
        ...
        public void TestRoundTripFidelity()
        {
            ...
            // Verify fidelity
            XunitAssert.Equal(original.Role, restored.Role);
            XunitAssert.Equal(original.MessageId, restored.MessageId);
            XunitAssert.Equal((original.Contents[0] as TextContent)?.Text, (restored.Contents[0] as TextContent)?.Text);
            ...
            XunitAssert.Equal("successful", result.round_trip);
        }
        ...
        public void TestBatchMessages()
        {
            ...
            XunitAssert.Equal(3, result.processed_count);
        }
        ...
        public void TestThreadMessages()
        {
            ...
            XunitAssert.Equal(3, result.thread_length);
        }
    }
}
