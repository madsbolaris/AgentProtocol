...
/**
 * Integration tests for Agent Protocol endpoints with EchoBot.
 *
 ...
 *
 * Matches the .NET EchoBotIntegrationTests.cs implementation.
 */
describe('Agent Protocol Integration Tests', () => {
  let app: express.Application;
  const echoBotUrl = process.env.ECHOBOT_URL || 'http://localhost:3980';
  ...
  beforeAll(async () => {
    ...
    if (process.env.USE_LIVE_SERVER === 'true') {
      ...
      app = express(); // Placeholder, supertest will use the URL
    } else {
  ...
  describe('Health Check', () => {
    it('should return 200 OK with status', async () => {
      const response = await request(echoBotUrl)
        .get('/health')
        .expect(200);
  ...
  describe('XML Test Data Processing', () => {
     ...
     */
    function xmlToChatMessage(xmlContent: string): any {
  ...
  describe('XML Results Whitespace Validation', () => {
    const resultsBase = path.join(testDataDir, 'results', 'echobot');
