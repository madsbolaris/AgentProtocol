...
PORTS_FILE="$PROJECT_ROOT/echo-bot-ports.json"

# Check if ports file exists
if [ ! -f "$PORTS_FILE" ]; then
    echo "Error: echo-bot-ports.json not found at project root"
    exit 1
fi
...
cd "$PROJECT_ROOT/python/samples/agents/echo-bot"
PORT=$PYTHON_PORT python src/start_server.py > "$LOG_DIR/python-echo-bot.log" 2>&1 &
PYTHON_PID=$!
echo "  Python echo bot PID: $PYTHON_PID"

# Start .NET echo bot
echo "Starting .NET echo bot on port $DOTNET_PORT..."
cd "$PROJECT_ROOT/dotnet/samples/agents/EchoBot"
PORT=$DOTNET_PORT dotnet run > "$LOG_DIR/dotnet-echo-bot.log" 2>&1 &
DOTNET_PID=$!
echo "  .NET echo bot PID: $DOTNET_PID"

# Start TypeScript echo bot
echo "Starting TypeScript echo bot on port $TYPESCRIPT_PORT..."
cd "$PROJECT_ROOT/typescript/samples/echo-bot"
PORT=$TYPESCRIPT_PORT npm start > "$LOG_DIR/typescript-echo-bot.log" 2>&1 &
TYPESCRIPT_PID=$!
